...
# Year'ı 0 olan satırları filtreleme
df = df[df['Year'] != 0]
...
plt.savefig(os.path.join(output_dir, 'movies_per_month.png'))
plt.close()

# Year_Diff için Histogram
plt.figure(figsize=(10, 6))
sns.histplot(df['Year_Diff'], bins=30, kde=True)
plt.title('Distribution of Year Difference')
plt.xlabel('Year Difference')
plt.ylabel('Frequency')
plt.savefig(os.path.join(output_dir, 'year_diff_distribution.png'))
plt.close()

# Year_Diff sütunundaki her bir unique değeri ve ilgili satırları yazdırma
unique_year_diff = df['Year_Diff'].unique()
for value in unique_year_diff:
    print(f"Year_Diff: {value}")
    print(df[df['Year_Diff'] == value])
    print("-" * 40)

...
Still simple yet much faster implementation: 1.5 sec for 2000 docs (1333 doc/sec)
    * Vocabulary build 33K doc/s
    * Parameter fit 20K doc/s
    * Prediction 8K doc/s

Trick is not to calculate total frequency by calling `.values` on dict in Probability class
"""
...
class Probability:
    ...
    def add(self, w: str, count: int = 1):
        ...
        self.total_term_freq += count

    def proba(self, w: str) -> float:
        return (self.freq.get(w, 0) + self.alpha) / (
            self.total_term_freq + self.alpha * (self.N + 1)
        )
...
def main():
    ...
    print(
        f"Accuracy (test) by Naive Bayesian classifier is {accuracy_score(test_df['label'], y_pred)}"
    )
    print(
        f"F1 (test) by Naive Bayesian classifier is {f1_score(test_df['label'], y_pred)}"
    )
